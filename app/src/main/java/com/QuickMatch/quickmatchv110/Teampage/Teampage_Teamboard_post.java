...
public class Teampage_Teamboard_post extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.teampage_teamboard_post);

        Intent getIntent = getIntent();
        myteam = getIntent.getParcelableExtra(Teaminfo.team);

        title = findViewById(R.id.teampage_board_postitle);
        content = findViewById(R.id.teampage_board_content);
        submit = findViewById(R.id.teampage_board_submit);
        ...
        submit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (mAuth.getCurrentUser() != null) {
                    String postID = mStore.collection(TeamPostID.board).document().getId();
                    ...
                    mStore.collection(Teaminfo.team).document(myteam.getTeamID())
                            .collection(TeamPostID.board).document(postID).set(data, SetOptions.merge());
                    finish();
                }else{
                    Toast.makeText(Teampage_Teamboard_post.this, "회원 정보가 없습니다.", Toast.LENGTH_SHORT).show();
                }
            }
        });
