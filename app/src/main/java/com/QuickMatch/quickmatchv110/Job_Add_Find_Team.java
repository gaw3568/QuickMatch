...
public class Job_Add_Find_Team extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.job_add_find_team);

        title = findViewById(R.id.add_find_team_title);
        contents = findViewById(R.id.add_find_team_content);
        ...
        findViewById(R.id.add_find_team_submit).setOnClickListener(this);
    ...
    public void onClick(View view) {
        switch (view.getId()) {//뷰들의 아이디를 가져옴
            case R.id.j_city://첫번째 지역 버튼을 눌렀을때(서울 경기 등등)
                AlertDialog.Builder citydialog = new AlertDialog.Builder(Job_Add_Find_Team.this);
            ...
            case R.id.j_state://두번째 지역 버튼을 눌렀을때(중랑구, 남양주시)
                AlertDialog.Builder statedialog = new AlertDialog.Builder(Job_Add_Find_Team.this);
            ...
            case R.id.j_position://팀 수준 버튼을 눌렀을때
                AlertDialog.Builder positionDialog = new AlertDialog.Builder(Job_Add_Find_Team.this);
            ...
            case R.id.add_find_team_submit:
                if(mAuth.getCurrentUser() != null) {         // 게시글 작성
                    String postID = mStore.collection(Job_F_Team_Info.jobFindTeam).document().getId();
                    Map<String, Object> data = new HashMap<>();
                    data.put(Job_F_Team_Info.userID, mAuth.getCurrentUser().getUid());
                    data.put(Job_F_Team_Info.nickname, nickname);
                    data.put(Job_F_Team_Info.title, title.getText().toString());
                    data.put(Job_F_Team_Info.city, j_city.getText().toString());
                    data.put(Job_F_Team_Info.state, j_state.getText().toString());
                    data.put(Job_F_Team_Info.position, j_position.getText().toString());
                    data.put(Job_F_Team_Info.contents, contents.getText().toString());
                    data.put(Job_F_Team_Info.timestamp, FieldValue.serverTimestamp());
                    mStore.collection(Job_F_Team_Info.jobFindTeam).document(postID).set(data, SetOptions.merge());
                    Toast.makeText(Job_Add_Find_Team.this, "작성 완료", Toast.LENGTH_SHORT).show();
                    finish();
                    break;
                }
        }
